fname = "ref"
fname_Front = fname + ".exr"
# fname_Back = fname + "_B.exr"
fname_Back = "trt.exr"
output_file = fname
...
imgF = np.log1p(cv2.imread(fname_Front, -1))
...
titles = ["Red (front)", "Green (front)", "Blue (front)", "Red (back)", "Green (back)", "Blue (back)"] if mode == 0 else ["Front", "Back"]
if mode == 0:
...
for i in range(0, m):
    ...
    title(titles[i], y=1.15, fontsize=18)
